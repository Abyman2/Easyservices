...
@Component
public class CarScraper extends BaseScraper {
    public CarScraper(ListingNormalizer n, @Value("${discovery.browser.headless:true}") boolean h,
                      @Value("${discovery.browser.timeout-seconds:20}") long t,
                      @Value("${discovery.user-agent:EasyServiceDiscovery/1.0}") String u) {
        super(n, h, t, u);
    }
    protected ListingCategory category() { return ListingCategory.CAR; }
}
